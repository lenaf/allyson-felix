...
const UpcomingFestival = ({ festival }: { festival: IUpcomingFestival }) => (
  <div>
    ...
    <div className="mb-2">
      {festival.theatreName && (
        <div className="text-white text-lg">{festival.theatreName}</div>
      )}
      ...
      {festival?.dates && (
        <div className="prose">
          <PortableText value={festival?.dates} />
        </div>
      )}
      {festival.ticketUrl && (
        <LinkAsButton
          className="h-6 min-h-6 text-lg gap-2 p-0"
...
const Screenings = ({
  ...
  pastFestivals: IPastFestival[];
  upcomingFestivals: IUpcomingFestival[];
}) => {
  return (
    <Section
      id="screenings"
    ...
    >
      ...
      <div className="flex flex-wrap lg:grid lg:grid-cols-3  gap-x-16 gap-y-8">
        ...
        <div className="mb-4 max-w-[416px]">
